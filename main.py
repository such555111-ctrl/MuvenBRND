...
log = logging.getLogger("muven_brnd")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "MÚVEN BRND.: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "♦ *MÚVEN BRND. — Yakut Streetwear*\n\n"
        "Добро пожаловать в MÚVEN BRND.! Зип-худи, футболки, лонгсливы, шопперы и "
        "аксессуары с якутскими орнаментами и национальными принтами.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ MV-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от MÚVEN BRND. по заказу MV-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от MÚVEN BRND. по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
